...
module.exports = function (RED) {
    function Generator(config) {
        ...
        node.on('input', function (msg) {
            ...
            try {
                ...
                console.error(msg.protobufType, node.protofile.protopath);

                let hook = msg.payload.hook;

                data = mocker({
                    cmd: `${msg.protobufType}`,
                    entry: path => path.resolve(`${node.protofile.protopath}`),
                    hook: {
                        [`${msg.protobufType}`]: (source, Mock) => {
                            if (hook != undefined) {
                                Object.assign(source, hook);
                            }
                        }
                    },
                })
    ...
    RED.nodes.registerType("generator", Generator);
}